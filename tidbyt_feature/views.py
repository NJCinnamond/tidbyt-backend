...
from tidbyt_feature.serializers import FeatureCreateSerializer, FeatureUpdateSerializer, FeatureReadSerializer
from tidbyt_feature.services import FeatureService, FeatureDeleteFailure, FeatureUpdateFailure, FeatureCreateFailure
...
class FeatureViewSet(FeatureReadWriteSerializerMixin, viewsets.ViewSet):
    ...
    def retrieve(self, request, pk=None): #GET to /features/{pk}
        try:
            ...
            return Response({'error': "Error reading feature: {}".format(e)}, status=status.HTTP_404_NOT_FOUND)
    ...
    def create(self, request): # POST to /features
        ...
        try:
            ...
            return Response({'error': "Error creating feature: {}".format(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
            return Response({'error': "Unknown error: {}".format(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def update(self, request, pk=None): # PUT to /features/{pk}
        try:
            ...
            return Response({'error': "Error updating feature: {}".format(e)}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            return Response({'error': "Error updating feature: {}".format(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
            return Response({'error': "Unknown error: {}".format(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def destroy(self, request, pk=None): # Delete to /features/{pk}
        try:
            ...
            return Response({'error': "Error deleting feature: {}".format(e)}, status=status.HTTP_404_NOT_FOUND)
        
        try:
            feature_svc = FeatureService()
            feature_svc.delete_feature(
                queryset, 
                request.data.get('uninstall_from_all_devices')
            )
